...
ggplot(data=Drake)+
  ...
  ggtitle("Close Friends by State")+
...
train.idx <- sample(x=1:nrow(Drake), size=floor(.8*nrow(Drake)))
...
finalforest<- randomForest(Friends ~ Hometown + Political + Religious + Basketball + Baseball + Watch.Sports
                           + Social.Confidence + Introvert.Extravert + Drink + Smoke + Nicotine + Lift
                           , data=train.df, ntree= 1000, mtry=4, importance=TRUE)
print(finalforest)
...
liftmodel = glm(Lift~ Political + Smoke + Drink + Religious + Basketball + Baseball + Watch.Sports + Wealth.Background 
                + Social.Confidence + Introvert.Extravert
                ,family = binomial(link = "logit"))
summary(liftmodel)

liftmodel = glm(Lift~ Political + Smoke + Drink + Religious + Basketball + Baseball + Watch.Sports 
                + Social.Confidence + Introvert.Extravert
                ,family = binomial(link = "logit"))
summary(liftmodel)

liftmodel = glm(Lift~ Political + Smoke + Drink + Religious + Baseball + Watch.Sports 
                + Social.Confidence + Introvert.Extravert
                ,family = binomial(link = "logit"))
summary(liftmodel)

liftmodel = glm(Lift~ Political + Smoke + Drink + Religious + Baseball+ Social.Confidence + Introvert.Extravert
                ,family = binomial(link = "logit"))
summary(liftmodel)

liftmodel = glm(Lift~ Political + Smoke + Drink + Religious + Baseball+ Social.Confidence
                ,family = binomial(link = "logit"))
summary(liftmodel)

liftmodel = glm(Lift~ Political + Smoke + Drink + Religious + Social.Confidence
                ,family = binomial(link = "logit"))
summary(liftmodel)

liftmodel = glm(Lift~ Political + Drink + Religious + Social.Confidence
                ,family = binomial(link = "logit"))
summary(liftmodel)

liftmodel = glm(Lift~ Political + Religious + Social.Confidence
                ,family = binomial(link = "logit"))
summary(liftmodel)
...
for (i in namevector){
  name1 = Drake[Name == i,]
  print(paste0(i,": ",round((exp(predict(model2,name1))/(exp(predict(model2,name1))+1))*100,digits = 2),"%"))
}
